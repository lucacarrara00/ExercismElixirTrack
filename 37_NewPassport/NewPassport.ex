defmodule NewPassport do
  ...
  def get_new_passport(now, birthday, form) do
    with {:ok, timestamp} <- enter_building(now),
         ...
         {:ok, verification} <- stamp_form(timestamp, manual.(birthday), form) do
      {:ok, get_new_passport_number(timestamp, manual.(birthday), verification)}
    else
      {:coffee_break, _} ->
        {:retry, NaiveDateTime.add(now, @seconds)}
